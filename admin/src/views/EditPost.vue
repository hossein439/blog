<template>
    <div id="create-post">
        <div class="grid grid-cols-10 px-12 my-10">
            <div class="col-span-7 rounded-lg py-10 dark:bg-[#171717] bg-white">
                <div class="px-12">
                    <div class="grid grid-cols-2 items-start justify-between">
                        <div class="flex">
                            <button @click="openWindow($event)"
                                class="relative cursor-pointer border-2 text-zinc-300 border-[#525252] rounded-md py-1.5 px-3 mr-3 hover:bg-zinc-800 transition-all">
                                {{ AddCoverTxt }}
                                <input ref="coverImageElement" @change="previewImage($event)" type="file"
                                    name="coverImage" hidden class="absolute inset-0 opacity-0">
                            </button>
                            <button v-show="isShowPreview" @click="removePreview()"
                                class="capitalize text-red-600 py-1.5 px-3 dark:hover:bg-zinc-800 hover:bg-zinc-100 transition-all rounded-md">remove</button>
                        </div>
                        <span v-show="isShowPreview"
                            class="inline-block ml-auto w-36 h-24 border rounded border-dashed border-gray-500">
                            <img ref="preview" class="block w-full h-full object-cover" src="" alt="" />
                        </span>
                    </div>
                    <div class="my-7">
                        <input ref="titlePostElement" v-model="titlePost" id="title" type="text"
        ...
        <!-- <div class="flex px-12">
            <button @click="createPost(true)"
                class="px-3 py-1.5 bg-indigo-800 rounded-md font-light hover:opacity-80 transition-all mr-6">Publish</button>
            <button @click="createPost(false)"
                class="px-3 py-1.5 hover:bg-indigo-600 rounded-md font-light transition-all">Save draft</button>
        </div> -->
...
Post.read({
    ...
    titlePost.value = singlePost.value.title;
    tagsPost.value = singlePost.value.tags;
    editorData.value = singlePost.value.body;
    isShowPreview.value = true;
    AddCoverTxt.value = 'Change';
    preview.value.src = `${require('../../../uploads/' + singlePost.value.coverImage)}`;
}).catch(err => {
    console.log(err)
})
...
function createPost(isPublished) {
    ...
    console.log(singlePost.value._id)
    
    Post.update({id:singlePost.value._id} ,formData)
        .then(res => {
            console.log(res);
        }).catch(err => {
            console.log(err);
